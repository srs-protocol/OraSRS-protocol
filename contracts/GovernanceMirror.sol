...
contract GovernanceMirror is LzApp {
    ...
    function _handleProposalMessage(uint16 _srcChainId, bytes memory _payload) internal {
        ...
        if (sourceProposalToMirror[mirrorKey] == 0) {
            ...
            proposals[mirrorProposalId] = Proposal({
                id: mirrorProposalId,
                proposer: proposalMsg.proposer,
                startTime: proposalMsg.startTime,
                endTime: proposalMsg.endTime,
                votesFor: 0,
                votesAgainst: 0,
                votesAbstain: 0,
                requiredQuorum: 0,  // 镜像提案不设置法定人数
                description: string(abi.encodePacked("[MIRROR] ", proposalMsg.description)),
                proposalType: proposalMsg.proposalType,
                state: ProposalState.Pending,  // 可能需要根据原提案状态设置
                targets: proposalMsg.targets,
                values: proposalMsg.values,
                calldatas: proposalMsg.calldatas,
                sourceChainId: _srcChainId,
                sourceProposalId: proposalMsg.sourceProposalId
            });
    ...
    function _handleVoteMessage(uint16 _srcChainId, bytes memory _payload) internal {
        ...
        if (!votes[mirrorProposalId][voteMsg.voter].hasVoted) {
            votes[mirrorProposalId][voteMsg.voter] = Vote({
                hasVoted: true,
                support: voteMsg.support,
                votes: 100  // 简化：固定100票
            });
