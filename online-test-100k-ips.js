...
async function runOnlineTest() {
  ...
  // 分批处理，避免对API造成过大压力
  const batchSize = 100; // 每批处理100个请求
  ...
  for (let i = 0; i < testIPs.length; i += batchSize) {
    ...
    console.log(`进度: ${processed}/${testIPs.length}, 本批次耗时: ${Date.now() - startTime}ms`);
  ...
  const report = {
    test_config: {
      ...
      test_type: '100000_ip_online_test',
  ...
  const logFileName = `online-test-100k-ips-${timestamp}.json`;
  ...
  const summaryLog = {
    test_summary: {
      test_name: '100000 IP Online API Test',
      total_ips: testIPs.length,
      target_api: 'https://api.orasrs.net',
      successful_queries: successCount,
      failed_queries: errorCount,
  ...
  const summaryFileName = `online-test-100k-ips-summary-${timestamp}.json`;
...
// 运行测试
console.log('注意：此测试将向 api.orasrs.net 发送大量请求，请确保您有权限执行此操作');
runOnlineTest()
  .then(() => {
    console.log('\n100000个IP在线查询测试完成！');
    process.exit(0);
