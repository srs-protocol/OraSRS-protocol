...
async function testGasSubsidy() {
  ...
  // 获取GasSubsidy合约地址（从all-deployments.json）
  const deploymentInfo = {
    gasSubsidyAddress: "0x5eb3Bc0a489C5A8288765d2336659EbCA68FCd00"
  };

  // 连接到GasSubsidy合约
  const GasSubsidy = await ethers.getContractFactory("GasSubsidy");
  const gasSubsidy = GasSubsidy.attach(deploymentInfo.gasSubsidyAddress);
  ...
  try {
    ...
    console.log("✓ 重复发放被正确阻止:", error.reason || error.message);
  }

  // 给用户2发放补贴
  console.log("\n给用户2发放Gas补贴...");
  try {
    const subsidyTx2 = await gasSubsidy.subsidize(user2.address);
    await subsidyTx2.wait();
    console.log("✓ 用户2 Gas补贴发放成功");
  } catch (error) {
    console.log("✗ 用户2 Gas补贴发放失败:", error);
  }
  ...
  try {
    const signers = await ethers.getSigners();
    const user3 = signers[6];
    const user4 = signers[7];
    const user5 = signers[8];
    const batchTx = await gasSubsidy.batchSubsidize([user3.address, user4.address, user5.address]);
    await batchTx.wait();
    console.log("✓ 批量发放成功");
  } catch (error) {
    console.log("✗ 批量发放失败:", error);
  }
