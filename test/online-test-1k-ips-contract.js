/**
 * OraSRS 1000个IP在线查询测试脚本（使用合约接口，小规模验证）
...
async function runOnlineTest() {
  ...
  try {
    ...
    console.log(`   ✓ 连接成功 - 链ID: ${network.chainId}, 当前区块: ${blockNumber}`);
  } catch (error) {
...
// 运行测试
console.log('注意：此测试将向 OraSRS 协议链发送查询请求，请确保您有权限执行此操作');
runOnlineTest()
  .then(() => {
    console.log('\n1000个IP在线合约查询测试完成！');
    process.exit(0);
