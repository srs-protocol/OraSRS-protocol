// test/orasrs-contract-interaction.cjs
...
async function testContractInteraction() {
  ...
  // 获取部署的合约地址
  const deploymentInfo = await import('../deployment-info.json', { assert: { type: 'json' } });
  console.log("部署的合约地址:", deploymentInfo.default);
  
  // 创建合约实例
  const threatIntelContract = new ethers.Contract(
    deploymentInfo.default.threatIntelContract, 
    threatIntelABI, 
    provider
  );
  
  const securityActionContract = new ethers.Contract(
    deploymentInfo.default.securityActionContract, 
    securityActionABI, 
    provider
  );
