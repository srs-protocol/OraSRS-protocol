...
func (c *SecurityRiskAssessmentContract) submitThreatReport() ([]byte, error) {
	// 验证调用者权限（现在任何注册节点都可以提交，包括自动注册）
	if err := c.onlyRegisteredNode(); err != nil {
		return nil, fmt.Errorf("only registered nodes can submit threat reports: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to marshal threat attestation: %v", err)
	}

	attestationKey := ThreatAttestationKey + reportID
	err = c.Put([]byte(attestationKey), attestationBytes)
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to save threat attestation: %v", err)
	}

	// 更新节点的威胁报告计数
	node, err := c.getNodeByAddress(c.Caller)
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to get node: %v", err)
	}
...
func (c *SecurityRiskAssessmentContract) validateThreatReport(attestation ThreatAttestation) error {
	// 验证IP格式
	...
	if err != nil {
		return fmt.Errorf("SecurityRiskAssessmentContract: failed to mark threat report as processed: %v", err)
	}
...
// isValidIP 验证IP地址格式
func (c *SecurityRiskAssessmentContract) isValidIP(ip string) bool {
	// 简单的IP格式验证
...
func (c *SecurityRiskAssessmentContract) isThreatReportDuplicate(key string) bool {
	_, err := c.Get([]byte(key))
	return err == nil
}

// addToGlobalThreatList 添加到全局威胁列表
func (c *SecurityRiskAssessmentContract) addToGlobalThreatList(ip string, level ThreatLevel) error {
	// 获取当前全局威胁列表
	...
	if err != nil {
		return fmt.Errorf("SecurityRiskAssessmentContract: failed to unmarshal threat list: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("SecurityRiskAssessmentContract: failed to marshal updated threat list: %v", err)
	}

	err = c.Put([]byte(GlobalThreatListKey), updatedBytes)
	if err != nil {
		return fmt.Errorf("SecurityRiskAssessmentContract: failed to save updated threat list: %v", err)
	}
...
func (c *SecurityRiskAssessmentContract) getGlobalThreatList() ([]byte, error) {
	threatListBytes, err := c.Get([]byte(GlobalThreatListKey))
	if err != nil {
...
func (c *SecurityRiskAssessmentContract) verifyThreatReport() ([]byte, error) {
	// 仅验证节点可调用
	...
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to unmarshal attestation: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to marshal verification result: %v", err)
	}

	err = c.Put([]byte(verificationKey), resultBytes)
	if err != nil {
		return nil, fmt.Errorf("SecurityRiskAssessmentContract: failed to save verification result: %v", err)
	}
...
func (c *SecurityRiskAssessmentContract) getThreatReport() ([]byte, error) {
	reportIDBytes, exists := c.Args["report_id"]
	if !exists || len(reportIDBytes) == 0 {
		return nil, fmt.Errorf("report_id is required")
	}
...
// onlyRegisteredNode 仅注册节点可调用（更宽松的访问控制）
func (c *SecurityRiskAssessmentContract) onlyRegisteredNode() error {
	// 获取节点信息，如果不存在则创建一个新节点
	node, err := c.getNodeByAddress(c.Caller)
	if err != nil {
		// 如果节点不存在，自动创建一个新节点（宽松注册）
		newNode := Node{
			NodeAddress:     c.Caller,
			StakeAmount:     0, // 无质押
			StakeStart:      c.TxTimeStamp,
			ReputationScore: 50, // 初始声誉分数
			Status:          Registered,
			NodeId:          fmt.Sprintf("auto_%s", c.Caller[:8]),
			BusinessLicense: "open_registration",
			FilingNumber:    "open_registration",
			ChallengeCount:  0,
			ChallengesWon:   0,
			ChallengesLost:  0,
			LastSeen:        c.TxTimeStamp,
			IsConsensusNode: false,
			IsThreatSensor:  true,      // 自动注册为威胁检测节点
			AgentVersion:    "2.0.0",   // 默认代理版本
		...
		err = c.saveNode(newNode)
		if err != nil {
			return fmt.Errorf("SecurityRiskAssessmentContract: failed to auto-register node: %v", err)
		}
	...
	// 如果节点已存在，确保其已注册
	if node.Status == Unregistered {
		node.Status = Registered
		node.ReputationScore = 50 // 设置基础声誉分数
		err = c.saveNode(*node)
		if err != nil {
			return fmt.Errorf("SecurityRiskAssessmentContract: failed to update node status: %v", err)
		}
	}
