...
async function testGasSubsidy() {
  ...
  const user1 = accounts[1];   // 0x70997970C51812dc3A010C7d01b50e0d17dc79C8
  ...
  console.log("\n给用户1发放Gas补贴...");
  try {
    const subsidyTx = await gasSubsidy.connect(deployer).subsidize(user1.address);
    ...
    // 再次检查用户1的已领取状态
    const hasClaimedAfter = await gasSubsidy.hasClaimed(user1.address);
