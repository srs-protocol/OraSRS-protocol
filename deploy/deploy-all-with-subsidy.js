...
async function main() {
  ...
  // 部署GasSubsidy合约 (需要Owner和Relayer地址作为参数)
  console.log("\n6. 部署GasSubsidy合约...");
  const GasSubsidy = await ethers.getContractFactory("GasSubsidy");
  const gasSubsidy = await GasSubsidy.deploy(deployer.address, deployer.address); // 使用部署者作为Owner和Relayer地址
  await gasSubsidy.waitForDeployment();
  console.log("GasSubsidy 合约部署地址:", await gasSubsidy.getAddress());
  ...
  const deploymentInfo = {
    ...
    gasSubsidyAddress: await gasSubsidy.getAddress(),
  ...
  // 向GasSubsidy合约注入资金
  console.log("\n正在向GasSubsidy合约注入资金...");
  const fundingAmount = ethers.parseEther("1000.0"); // 1000 ETH/BNB/原生代币
  const fundingTx = await deployer.sendTransaction({
    to: await gasSubsidy.getAddress(),
    value: fundingAmount
  });
  await fundingTx.wait();
  console.log("✓ 已向GasSubsidy合约注入", ethers.formatEther(fundingAmount), "原生代币");
}
