...
## 性能测试方法
为了验证协议性能，我们提供了多种测试脚本，可复现协议的性能指标：

1. **速率限制测试**：验证客户端是否正确实现了20r/s的速率限制
2. **协议链连接测试**：验证与OraSRS协议链的连接能力
3. **IP查询性能测试**：测试单个和批量IP查询的响应时间和吞吐量
4. **缓存性能测试**：验证缓存机制的有效性
5. **分离延迟测试**：分别测量本地命中、同区域链查、跨洲链查的延迟
6. **精度/召回率测试**：在公开IDS数据集上验证威胁检测的准确性
7. **抗女巫攻击测试**：验证系统对女巫攻击的抵抗能力

运行综合测试：
```bash
node latency-test-separated.js
```

测试完成后将生成详细的性能报告，包含QPS、响应时间、缓存命中率、精确率、召回率、误报率、抗女巫攻击能力等关键指标。
...
### 6. 测试与日志记录
- **性能测试**: 提供一键性能测试脚本，可复现协议性能指标
- **日志记录**: 所有系统日志均经过脱敏处理，不包含敏感信息
- **访问控制**: 实施速率限制（20r/s）和连接限制（每IP最多10个并发连接）
- **审查支持**: 标准测试日志保存在 `logs/` 目录中，供外部审查
